...
export default function ProxyProfiles() {
  ...
  // A SET, not one id.
  //
  // This was a single "which row is refreshing" value, so starting a second check stopped
  // the first one's spinner, and when the first finished it cleared the flag out from under
  // the second — two probes running and neither of them showing it.
  const [refreshingIds, setRefreshingIds] = useState<Set<number>>(new Set());
  const [refreshingAll, setRefreshingAll] = useState(false);
  const isRefreshing = (id: number) => refreshingIds.has(id);
  const markRefreshing = (id: number, on: boolean) =>
  ...
  const refreshOne = async (id: number, quiet = false): Promise<void> => {
    ...
    try {
      ...
      // This row only. Every other row keeps what it had, including ones still checking.
      setRows((prev) => prev.map((r) => (r.id === id ? { ...updated, usedBy: r.usedBy } : r)));
    } catch {
      if (!quiet) toast({ title: t.failedToRefresh, variant: "destructive" });
    } finally {
  ...
  /**
   * Check them all, but as the rows they belong to — not as one long wait.
   *
   * This used to call a server endpoint that probed every proxy and returned when the last
   * one finished, then reloaded the whole list: nothing moved for as long as the slowest
   * proxy took, and then every row blanked and redrew at once. The information arrives one
   * proxy at a time, so it is shown one proxy at a time — each row spins on its own and
   * settles the moment its own answer comes back.
   *
   * A few at a time rather than all at once: these are real connections through real
   * proxies, and firing thirty of them together makes the slow ones time out.
   */
            ...
            <Card key={p.id} className="border-border shadow-sm">
              <CardHeader className="pb-2 bg-muted/20 border-b border-border flex-row items-center justify-between py-3 px-4 gap-3">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  <Button variant="ghost" size="icon" className="h-7 w-7" title={t.refreshExitOne} onClick={() => refreshOne(p.id)} disabled={isRefreshing(p.id)}>
                    <RefreshCw className={`h-4 w-4 ${isRefreshing(p.id) ? "animate-spin" : ""}`} />
                  </Button>
