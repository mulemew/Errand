...
const BASE = import.meta.env.BASE_URL.replace(/\/$/, "");
  ...
  /**
   * The passphrase is typed here and nowhere else.
   *
   * It is not stored — not in an environment variable, not in the database. That is the
   * whole point of using a passphrase rather than the instance's own encryption key: the
   * backup has to open somewhere that key has never been. Which is also why it is asked for
   * twice, since a typo in something that is never saved produces a file nobody can open.
   */
  function BackupSection() {
    ...
    const canExport = pass1.length >= 8 && pass1 === pass2 && !exporting;
    ...
    const handleExport = async () => {
      ...
      try {
        const res = await fetch(`${BASE}/api/backup/export`, {
          ...
          headers: { "Content-Type": "application/json" },
          credentials: "same-origin",
          body: JSON.stringify({ passphrase: pass1, includeSessions }),
        });
        if (!res.ok) {
          const data = (await res.json().catch(() => ({}))) as { error?: string };
          throw new Error(data.error ?? "");
        }
        // The response IS the file. Saving it straight from the blob keeps it off any disk
        // but the one you chose.
        ...
        setPass1("");
        setPass2("");
        toast({ title: t.backupExported, description: t.backupWarnLostPassphrase, variant: "success" });
      } catch (err) {
        toast({ title: t.backupFailed, description: err instanceof Error ? err.message : "", variant: "destructive" });
      } finally {
        setExporting(false);
      }
    };
    ...
    const handleImport = async () => {
      ...
      try {
        ...
        const res = await fetch(`${BASE}/api/backup/import`, {
          ...
          headers: { "Content-Type": "application/json" },
          credentials: "same-origin",
          body: JSON.stringify({ passphrase: importPass, file: parsed }),
        });
        const data = (await res.json().catch(() => ({}))) as Record<string, number | string>;
        if (!res.ok) throw new Error(String(data.error ?? ""));
        setImportPass("");
        setFile(null);
        toast({
          title: t.backupImported,
          description: t.backupImportedDetail
            .replace("{tasks}", String(data.tasks ?? 0))
            .replace("{fingerprints}", String(data.fingerprints ?? 0))
            .replace("{proxies}", String(data.proxies ?? 0))
            .replace("{credentials}", String(data.savedCredentials ?? 0))
            .replace("{sessions}", String(data.sessionProfiles ?? 0)),
          variant: "success",
        });
      } catch (err) {
        toast({ title: t.backupFailed, description: err instanceof Error ? err.message : "", variant: "destructive" });
    ...
    return (
      <div className="space-y-6">
        <div className="space-y-3">
          <div>
            <p className="text-sm font-medium">{t.backupExportTitle}</p>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{t.backupExportHint}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="bkPass1">{t.backupPassphrase}</Label>
              <Input
                id="bkPass1"
                type="password"
              ...
              />
              {tooShort && <p className="text-xs text-destructive">{t.backupPassphraseTooShort}</p>}
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="bkPass2">{t.backupPassphraseAgain}</Label>
              <Input
                id="bkPass2"
                type="password"
                autoComplete="new-password"
              ...
              />
              {mismatch && <p className="text-xs text-destructive">{t.backupPassphraseMismatch}</p>}
            </div>
          </div>
          <label className="flex items-start gap-2 text-xs text-muted-foreground cursor-pointer">
            ...
            />
            <span>
              <span className="text-foreground">{t.backupIncludeSessions}</span>
              <br />
              {t.backupIncludeSessionsHint}
            </span>
          </label>
        ...
        <div className="space-y-3 pt-4 border-t border-border">
          <div>
            <p className="text-sm font-medium">{t.backupImportTitle}</p>
            <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{t.backupImportHint}</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label htmlFor="bkFile">{t.backupChooseFile}</Label>
              <Input
                id="bkFile"
                type="file"
              ...
              />
              <p className="text-xs text-muted-foreground truncate">{file ? file.name : t.backupNoFile}</p>
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="bkImportPass">{t.backupPassphrase}</Label>
              <Input
                id="bkImportPass"
                type="password"
                autoComplete="off"
          ...
          <Button size="sm" variant="outline" disabled={!file || !importPass || importing} onClick={handleImport}>
            {importing ? (
              <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                {t.backupImporting}
    ...
    <>
      ...
      <div className="space-y-8 animate-in fade-in duration-500 max-w-2xl">
        ...
        <Card className="border-border shadow-sm">
          <CardHeader className="bg-muted/20 border-b border-border pb-4">
            <CardTitle className="text-base flex items-center gap-2">
              <Archive className="h-4 w-4 text-primary" /> {t.backupTitle}
            </CardTitle>
            <CardDescription className="text-xs mt-1">{t.backupHint}</CardDescription>
          </CardHeader>
          <CardContent className="pt-4">
            <BackupSection />
          </CardContent>
