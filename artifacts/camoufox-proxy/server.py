...
@app.get("/session/<sid>/cookies")
def session_cookies(sid):
    """Every cookie in this session's profile, whichever container holds it.

    Playwright's storageState() reports one container — the one it created — and a tab the
    person opened from the browser's own UI is not in it. Firefox implements a Playwright
    BrowserContext as a userContext, so those tabs land in the DEFAULT container and their
    cookies are invisible to the API. Measured: visiting a site in the automated tab and
    again in a hand-opened tab left two cookies under each container in the profile, and
    storageState() returned only its own two. The profile is a temp directory, so on close
    the other half was simply deleted — a login performed in a hand-opened tab did not
    survive, which for a browser whose whole purpose is to keep an identity is a hole.

    Reading the file is the only way to see all of it. It is copied first: Firefox holds the
    database open in WAL mode, and reading it in place gets a torn view or a lock.
    """
    with _lock:
        entry = _servers.get(sid)
    if not entry:
        return jsonify({"cookies": []}), 404
    proc = entry.get("proc")
    prof = profile_dir_for_session(entry.get("pgid"), getattr(proc, "pid", None))
    if not prof:
        return jsonify({"cookies": [], "reason": "profile not found"})
    src = os.path.join(prof, "cookies.sqlite")
    if not os.path.exists(src):
        return jsonify({"cookies": [], "reason": "no cookie database yet"})

    tmp = tempfile.mkdtemp(prefix="cookiescan-")
    try:
        ...
        print(f"[camoufox] could not read the cookie jar for {sid}: {e}", flush=True)
    ...
    for name, value, host, path, expiry, secure, http_only, sslot, oa in rows:
        ...
        out.append({
            ...
            # Not part of the storageState shape — the caller uses it to tell which of these
            # it already had, and drops it before saving.
            "_container": oa or "",
        })
...
def _release_entries(entries, why):
...
def release_all():
    ...
    with _lock:
        ...
        _servers.clear()
    _release_entries(entries, "release-all")
