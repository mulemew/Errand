  ...
  if (tokenDetection) {
    ...
    if (type === "reCAPTCHA") {
      ...
      const interactive = (await page
        .evaluate(() => {
          // 280x70, not 200x60. Measured on one host panel's login page: the INVISIBLE
          ...
          // form never submitted. Measured on that host panel, whose reCAPTCHA needs no
          // interaction at all.
      ...
      // one — measured on one host panel's login page at 1918x150 with nothing to solve. So
