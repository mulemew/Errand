...
export interface BrowserProvider {
  ...
  /**
   * Every cookie in the browser's own jar, not just the ones the automation context holds.
   *
   * Same blind spot as listOpenTabs, one layer down: a tab opened from the browser's UI
   * lives in a different container, and its cookies never appear in storageState().
   */
...
class CamoufoxProvider implements BrowserProvider {
  ...
  /**
   * The profile's whole cookie jar, via the sidecar. See readAllCookies on the interface.
   */
  async readAllCookies(): Promise<Array<Record<string, unknown>>> {
    const id = [...this._ids.values()][0];
    if (!id) return [];
    try {
      const res = await fetch(`${this.baseUrl}/session/${encodeURIComponent(id)}/cookies`, {
        signal: AbortSignal.timeout(10_000),
      });
      if (!res.ok) return [];
      const body = (await res.json()) as { cookies?: unknown };
      return Array.isArray(body.cookies) ? (body.cookies as Array<Record<string, unknown>>) : [];
    } catch {
      // Best effort: a browser must still close cleanly when this cannot be read.
      return [];
    }
      ...
      async (x: number, y: number): Promise<boolean> => {
        try {
          ...
          const res = await fetch(`${this.baseUrl}/sessions/${encodeURIComponent(id)}/os-click`, {
            ...
            // subprocess timeout guarding it there (90s), so a slow gesture is reported as
            ...
            //
            // 45s, because the gesture now aims at a DURATION inside the widget rather than
            // a step count: the same command list was timed at 5.9s and at 21.6s in the same
            // container minutes apart, so a fixed budget sized from one of those numbers is
            // wrong for the other.
