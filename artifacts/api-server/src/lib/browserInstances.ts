...
/**
 * Save what is open, on a timer, instead of only when you close it.
 *
 * Saving on close alone means everything since the last close is held in a temp profile
 * that exists only while the process does. A crashed browser, a killed sidecar, an app
 * restart — all of them threw away the logins and tabs from that whole sitting, and an app
 * restart is not rare: it is how this thing is deployed.
 *
 * Only browsers you keep (they have a profile row); a task's browser is dumped by the
 * runner when its run ends. Best-effort and quiet: this runs unattended and must never turn
 * a working browser into a failure.
 */
const AUTOSAVE_MS = Number(process.env.BROWSER_AUTOSAVE_MS ?? 120_000);
let autosaveTimer: NodeJS.Timeout | null = null;
const savingNow = new Set<string>();

async function autosaveInstance(inst: BrowserInstance): Promise<void> {
  if (inst.sessionProfileId == null || savingNow.has(inst.id)) return;
  savingNow.add(inst.id);
  try {
    const state = await Promise.race([
      dumpInstanceSession(inst),
      new Promise<null>((r) => setTimeout(() => r(null), SESSION_DUMP_TIMEOUT_MS)),
    ]);
    // Never overwrite a good session with an empty one. A dump that came back with nothing
    // is a browser that was busy or shutting down, not a browser that logged out.
    const cookies = (state as { cookies?: unknown[] } | null)?.cookies;
    if (!state || !Array.isArray(cookies) || cookies.length === 0) return;
    await saveSessionProfileState(inst.sessionProfileId, state, {
      ...
      openUrls: await collectTabsToRestore(inst),
    });
  } catch (err) {
    logger.debug({ err, id: inst.id }, "Periodic session save skipped");
  } finally {
    savingNow.delete(inst.id);
  }
}

function ensureAutosaveTimer(): void {
  if (autosaveTimer || AUTOSAVE_MS <= 0) return;
  autosaveTimer = setInterval(() => {
    const live = [...instances.values()].filter((i) => i.sessionProfileId != null);
    if (live.length === 0) {
      // Nothing to save and nothing to wake up for.
      if (autosaveTimer) clearInterval(autosaveTimer);
      autosaveTimer = null;
      return;
    }
    for (const inst of live) void autosaveInstance(inst);
  }, AUTOSAVE_MS);
 ...
 */
/**
 * The cookies the automation context cannot see, folded in.
 *
 * Firefox implements a Playwright BrowserContext as a userContext — a container — and a tab
 * the person opened from the browser's own UI is not in it. Measured: the same site visited
 * in the automated tab and again in a hand-opened one left two cookies under each container
 * in the profile, and storageState() returned only its own two. The profile is a temp
 * directory, so on close the other half was deleted with it, and a login performed in a
 * hand-opened tab did not survive the restart.
 *
 * The context's own cookies win on a conflict: they are the ones this browser was opened
 * with, and the ones a task would have used. Anything the jar has that they do not — which
 * is exactly the hand-opened tabs' half — is added.
 */
async function withProfileCookies(inst: BrowserInstance, state: unknown): Promise<unknown> {
  if (!state || typeof state !== "object") return state;
  const s = state as { cookies?: Array<Record<string, unknown>> };
  let jar: Array<Record<string, unknown>> = [];
  try {
    jar = (await inst.provider.readAllCookies?.()) ?? [];
  } catch {
    return state; // the jar is a bonus; never let it cost the dump
  }
  if (!jar.length) return state;

  const own = Array.isArray(s.cookies) ? s.cookies : [];
  const key = (c: Record<string, unknown>) => JSON.stringify([c.name, c.domain, c.path]);
  const have = new Set(own.map(key));
  const extra: Array<Record<string, unknown>> = [];
  for (const c of jar) {
    const { _container, ...cookie } = c; // the container tag is ours, not part of the shape
    void _container;
    if (!cookie.name || !cookie.domain) continue;
    if (have.has(key(cookie))) continue;
    have.add(key(cookie));
    extra.push(cookie);
  }
  if (!extra.length) return state;
  logger.info(
    { id: inst.id, fromContext: own.length, addedFromProfile: extra.length },
    "Folded in cookies the automation context could not see",
  );
  return { ...(state as object), cookies: [...own, ...extra] };
}
...
export async function dumpInstanceSession(inst: BrowserInstance): Promise<unknown | null> {
  ...
  if (inst.dumpStorageState) {
    try {
      return await withProfileCookies(inst, await inst.dumpStorageState());
    } catch (err) {
